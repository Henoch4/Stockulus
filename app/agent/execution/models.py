...
class OrderStatus(Enum):
    ...
    def from_exchange(cls, state: str) -> "OrderStatus":
        ...
        if mapped is None:
            logger.warning("Unmapped exchange order state %r — treating as PENDING", state)
            return cls.PENDING
        return mapped
...
class OrderRequest:
    ...
    # Units of `size`. "usd" (default): notional in quote currency (USDC) —
    # the executor converts to exchange-native units before submitting.
    ...
    # Declared effective leverage for this order (1.0 = no leverage). The risk
    # gate rejects any order whose declared leverage exceeds max_leverage.
    leverage: float | None = None
    # Intended price from signal generation (mid/last price). Used for
    # pre-trade fat-finger check on market orders.
